...
// Use the SD Fat library (not the SD wrapper) to access files on the SD Disk
// #include <SD.h>
#include <SdFat.h>
#include <SdFatUtil.h>
...
#define HTML_CLOSE F("</body></html>")
  ...
  // Write a error code to the Serial debug port.
  void numerror(int code) {
    Serial.print("error: ");
    Serial.println(code);
  }


  // find the entry for a given key in the table.
  // search for key plus a trailing item separator.
  ...
  // skip this word and find beginning of the next one.
  // if pointing to space: find the next starting position
  char *_ctSkipWord(char *text)
  {
  ...
  void setup() {
    ...
    if (!card.init(SPI_FULL_SPEED, 4)) numerror(ERR_CARDINIT);
    DEBUGTEXT(F("2"));

    // initialize a FAT volume
    if (!volume.init(&card)) numerror(ERR_VOLINIT);
    if (!root.openRoot(&volume)) numerror(ERR_OPENROOT);
    DEBUGTEXT(F("4"));
  ...
  void respondSystemInfo()
  {
    ...
    _client.print("Volume is FAT"); _client.println(volume.fatType(), DEC);
    _client.print("</pre>");
    _client.print(HTML_CLOSE);
  } // respondSystemInfo()
  ...
  void respondFileContent()
  {
    ...
    if (!_file.open(&root, _httpURI, O_READ)) {
      respondHeaderNotFound();
    }
    else {
      ...
      _client.print("Content-Length: "); _client.print(_file.fileSize()); _client.print("\r\n");
      ...
      int len;
      do {
        len = _file.read((uint8_t*)_readBuffer, 32);
        ...
        if (_client.available()) {
          ...
          if (now > timeout) {
            ...
            if (len > 0) {
              ...
              if (!_file.isOpen()) {
                if (!_file.open(&root, _httpURI, O_CREAT | O_WRITE | O_TRUNC)) {
                  webstate = PROCESS_ERR;
                } // if
              }  // if
              _file.write((uint8_t *)_readBuffer, len);
              _httpContentLen -= len;
            } // if
  ...
  void runRadioCommand(char *cmd, int16_t value) {
    if (strcmp(cmd, "freq") == 0) { radio.setFrequency(value); }
    else if (strcmp(cmd, "vol") == 0) { radio.setVolume(value); }
    else if (strcmp(cmd, "mute") == 0) { radio.setMute(value > 0); }
    else if (strcmp(cmd, "mono") == 0) { radio.setMono(value > 0); }
    else if (strcmp(cmd, "softmute") == 0) { radio.setSoftMute(value > 0); }
    else if (strcmp(cmd, "bassboost") == 0) { radio.setBassBoost(value > 0); }
  } // runRadioCommand()
  ...
  void runCommand(char cmd, int16_t value)
  {
    ...
    else if (cmd == 'm') { radio.setMute(value > 0); }
